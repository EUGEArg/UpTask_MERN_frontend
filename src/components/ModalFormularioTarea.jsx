...
const ModalFormularioTarea = () => {
    ...
    const params = useParams() //Extraer el proyecto
      
    const { modalFormularioTarea, handleModalTarea, mostrarAlerta, alerta, submitTarea, tarea} = useProyectos()

    useEffect(() =>{
        if(tarea?._id){ //Si encuentra una tarea lo coloca en el state
            setId(tarea._id)
        ...
        setId('')
        setNombre('') //Si no encuetra tarea queda vacío
        setNombre('')
        setDescription('')
        setFechaEntrega('')
        setPrioridad('')
        
    },[tarea]) //Cada vez que tarea cambia hace un render.

    const handleSubmit =async e => {
        e.preventDefault();

        if([nombre, description, fechaEntrega, prioridad].includes('')){ //Validación form tareas
            mostrarAlerta({
        ...
        await submitTarea({id, nombre, description, fechaEntrega, prioridad, proyecto: params.id})

        //vaciar el state luego de agregar tarea
        setId('')
        setNombre('')
        setDescription('')
        setFechaEntrega('')
